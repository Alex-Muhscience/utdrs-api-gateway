...
async def get_simulations(
    ...
    if status:
        ...
        return await simulation_service.get_simulations_by_scenario_type(
...
async def create_simulation(
    simulation: SimulationCreate, current_user: User = Depends(get_current_active_user)
):
    """
...
async def update_simulation(
    ...
    """
    simulation_service = SimulationService()
    updated_simulation = await simulation_service.update_simulation(
        simulation_id, simulation_update, current_user.id
    )
...
async def start_simulation(
    simulation_id: str, current_user: User = Depends(get_current_active_user)
):
    """
    Start a simulation.
...
async def generate_simulation_event(
    ...
    try:
        event_id = await simulation_service.generate_simulation_event(
            simulation_id, event_data
        )
        return {
            "status": "success",
            "message": f"Generated event for simulation {simulation_id}",
            "event_id": event_id,
        }
...
async def associate_alert_with_simulation(
    ...
    result = await simulation_service.associate_alert_with_simulation(
    ...
    return {
        "status": "success",
        "message": f"Associated alert {alert_id} with simulation {simulation_id}",
    }
