...
class AssetService:
    ...
    async def create_asset(self, asset_data: AssetCreate) -> str:
        ...
        try:
            ...
            if asset_data.name:
                ...
                if existing_asset:
                    logger.warning(
                        f"Asset with name '{asset_data.name}' already exists"
                    )
            ...
            if asset_data.ip_address:
                ...
                if existing_asset:
                    logger.warning(
                        f"Asset with IP '{asset_data.ip_address}' already exists"
                    )
                    return str(existing_asset["_id"])

            if asset_data.mac_address:
                existing_asset = await self.asset_repo.find_by_mac(
                    asset_data.mac_address
                )
                if existing_asset:
                    logger.warning(
                        f"Asset with MAC '{asset_data.mac_address}' already exists"
                    )
                    return str(existing_asset["_id"])
    ...
    async def update_asset(
        self, asset_id: str, asset_data: AssetUpdate
    ...
    async def update_asset_last_seen(self, asset_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error updating last seen timestamp for asset {asset_id}: {str(e)}"
            )
            raise

    async def add_vulnerability_to_asset(
        self, asset_id: str, vulnerability: str
    ) -> bool:
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Added vulnerability '{vulnerability}' to asset {asset_id}"
                )
            else:
    ...
    async def remove_vulnerability_from_asset(
        ...
        try:
            ...
            if result:
                logger.info(
                    f"Removed vulnerability '{vulnerability}' from asset {asset_id}"
                )
            else:
            ...
            logger.error(
                f"Error removing vulnerability from asset {asset_id}: {str(e)}"
            )
            raise

    async def get_assets_by_type(
        self, asset_type: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
        """Get assets by type."""
    ...
    async def get_assets_by_criticality(
        self, criticality: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
    ...
    async def get_assets_by_department(
        self, department: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
