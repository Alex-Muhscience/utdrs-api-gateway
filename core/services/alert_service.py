...
class AlertService:
    ...
    async def create_alert(
        self, alert_data: AlertCreate, user_id: Optional[str] = None
    ) -> str:
        """Create a new alert."""
        try:
    ...
    async def update_alert(
        self, alert_id: str, alert_data: AlertUpdate, user_id: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """Update an existing alert."""
        try:
            updated_alert = await self.alert_repo.update_alert(
                alert_id, alert_data, user_id
            )
            if updated_alert:
    ...
    async def assign_alert(self, alert_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error assigning alert {alert_id} to user {user_id}: {str(e)}"
            )
            raise
    ...
    async def get_alerts_assigned_to_user(
