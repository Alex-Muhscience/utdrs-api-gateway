...
class SimulationService:
    ...
    async def create_simulation(
        self, simulation_data: SimulationCreate, user_id: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            if simulation_data.name:
                existing_sim = await self.simulation_repo.find_by_name(
                    simulation_data.name
                )
                if existing_sim:
                    logger.warning(
                        f"Simulation with name '{simulation_data.name}' already exists"
                    )
                    return str(existing_sim["_id"])

            simulation_id = await self.simulation_repo.create_simulation(
            ...
            if simulation_data.target_assets:
                for asset_id in simulation_data.target_assets:
                    ...
                    if not asset:
                        logger.warning(f"Asset with ID {asset_id} not found")
                        raise ValueError(
                            f"Target asset with ID {asset_id} does not exist"
                        )

            updated_simulation = await self.simulation_repo.update_simulation(
                simulation_id, simulation_data, user_id
            )
    ...
    async def start_simulation(
        ...
        try:
            ...
            result = await self.simulation_repo.update_status(
                simulation_id, "running", user_id
            )
            if not result:
    ...
    async def stop_simulation(
        ...
        try:
            ...
            # Update simulation status
            result = await self.simulation_repo.update_status(
                simulation_id, "completed", user_id
            )
            if not result:
                logger.warning(f"Failed to update simulation {simulation_id} status")
                return False
    ...
    async def get_simulations_by_status(
        self, status: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
        """Get simulations by status."""
        return await self.simulation_repo.find_by_status(status, limit, skip)

    async def get_simulations_by_scenario_type(
        self, scenario_type: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
        """Get simulations by scenario type."""
        return await self.simulation_repo.find_by_scenario_type(
            scenario_type, limit, skip
        )
    ...
    async def update_simulation_results(
        self, simulation_id: str, results: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.warning(
                    f"Failed to update results for simulation {simulation_id}"
                )
            return result
        except Exception as e:
            logger.error(
                f"Error updating results for simulation {simulation_id}: {str(e)}"
            )
            raise

    async def generate_simulation_event(
        self, simulation_id: str, event_data: EventCreate
    ) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error generating event for simulation {simulation_id}: {str(e)}"
            )
            raise

    async def associate_alert_with_simulation(
        self, simulation_id: str, alert_id: str
    ) -> bool:
        """Associate an alert with a simulation."""
        try:
            result = await self.simulation_repo.add_alert_to_simulation(
                simulation_id, alert_id
            )
            if result:
                logger.info(
                    f"Associated alert {alert_id} with simulation {simulation_id}"
                )
            else:
                logger.warning(
                    f"Failed to associate alert {alert_id} with simulation {simulation_id}"
                )
            return result
        except Exception as e:
    ...
    async def _start_simulation_in_core_engine(self, simulation_id: str) -> bool:
        ...
        logger.info(
            f"Would start simulation {simulation_id} in core engine at {self.core_engine_url}"
        )
        return True
    ...
    async def _stop_simulation_in_core_engine(self, simulation_id: str) -> bool:
        ...
        logger.info(
            f"Would stop simulation {simulation_id} in core engine at {self.core_engine_url}"
        )
        return True
