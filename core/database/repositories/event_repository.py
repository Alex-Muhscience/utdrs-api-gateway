...
class EventRepository(BaseRepository):
    ...
    async def find_by_type(
        self, event_type: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
    ...
    async def find_by_ip(
        self, ip_address: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
        query = {"$or": [{"source_ip": ip_address}, {"destination_ip": ip_address}]}
    ...
    async def update_event(
        self, id: str, event_update: EventUpdate
    ...
    async def set_severity(self, id: str, severity: str) -> bool:
        ...
        result = await self.collection.update_one(
            {"_id": ObjectId(id)},
            {"$set": {"severity": severity, "updated_at": datetime.utcnow()}},
        )
        return result.modified_count > 0
    ...
    async def find_by_time_range(
        self, start_time: datetime, end_time: datetime, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
