...
class SimulationRepository(BaseRepository):
    ...
    async def find_by_scenario_type(
        self, scenario_type: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
        return await self.find_many(
            {"scenario_type": scenario_type}, limit=limit, skip=skip
        )

    async def find_by_target_asset(
        self, asset_id: str, limit: int = 100, skip: int = 0
    ) -> List[Dict[str, Any]]:
    ...
    async def find_scheduled(
        ...
        now = datetime.utcnow()
        query = {"status": "idle", "scheduled_start": {"$gte": now}}
    ...
    async def create_simulation(
        self, simulation: SimulationCreate, user_id: Optional[str] = None
    ) -> str:
