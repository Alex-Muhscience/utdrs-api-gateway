...
class DetectionRepository(BaseRepository):
    ...
    async def find_by_source(
        ...
        return await self.find_many(
            {"detection_source": detection_source}, limit=limit, skip=skip
        )
    ...
    async def create_rule(
        self, rule: DetectionRuleCreate, user_id: Optional[str] = None
    ) -> str:
        rule_dict = rule.dict()
    ...
    async def update_rule(
        self, id: str, rule_update: DetectionRuleUpdate, user_id: Optional[str] = None
    ...
    async def update_status(
        self, id: str, status: str, user_id: Optional[str] = None
    ) -> bool:
        if not ObjectId.is_valid(id):
            return False
        ...
        result = await self.collection.update_one(
            {"_id": ObjectId(id)}, {"$set": update_doc}
        )
    ...
    async def update_performance_metrics(
        self, id: str, metrics: Dict[str, Any]
    ) -> bool:
        if not ObjectId.is_valid(id):
            return False
